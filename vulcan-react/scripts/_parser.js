...
function parse(filePathOrPaths, parserOpts) {
  if (parserOpts === void 0) {
    parserOpts = exports.defaultParserOpts;
  }
...
function withCustomConfig(tsconfigPath, parserOpts) {
  ...
  if (error !== undefined) {
    ...
    var errorText =
      "Cannot load custom tsconfig.json from provided path: " +
      tsconfigPath +
      ", with error code: " +
      error.code +
      ", message: " +
  ...
  var _b = ts.parseJsonConfigFileContent(
      config,
      ts.sys,
      basePath,
      {},
...
var Parser = /** @class */ (function () {
  function Parser(program, opts) {
    var savePropValueAsString = opts.savePropValueAsString,
      shouldExtractLiteralValuesFromEnum =
        opts.shouldExtractLiteralValuesFromEnum,
      shouldRemoveUndefinedFromOptional =
        opts.shouldRemoveUndefinedFromOptional,
      shouldExtractValuesFromUnion = opts.shouldExtractValuesFromUnion;
  ...
  Parser.prototype.getComponentFromExpression = function (exp) {
    ...
    if (
      ...
      var component = this.checker.getSymbolAtLocation(
  ...
  Parser.prototype.getComponentInfo = function (
    ...
    if (componentNameResolver === void 0) {
    ...
    var _a = this.findDocComment(commentSource),
    ...
    if (propsType) {
      ...
      var defaultProps = this.extractDefaultPropsFromComponent(
        commentSource,
  ...
  Parser.prototype.extractPropsFromTypeIfStatefulComponent = function (type) {
    ...
    if (constructSignatures.length) {
      ...
      for (
        var _i = 0, constructSignatures_1 = constructSignatures;
        _i < constructSignatures_1.length;
        _i++
      ) {
  ...
  Parser.prototype.getMethodsInfo = function (type) {
    ...
    members.forEach(function (member) {
      if (!_this.isTaggedPublic(member) || !member) {
        return;
      }
      ...
      var description = ts.displayPartsToString(
        member.getDocumentationComment(_this.checker)
      );
      var returnType = _this.checker.typeToString(
  ...
  Parser.prototype.getParameterInfo = function (callSignature) {
    ...
    return callSignature.parameters.map(function (param) {
      var paramType = _this.checker.getTypeOfSymbolAtLocation(
        param,
      ...
      return {
        description:
          ts.displayPartsToString(
            param.getDocumentationComment(_this.checker)
          ) || null,
  ...
  Parser.prototype.getCallSignature = function (symbol) {
    var symbolType = this.checker.getTypeOfSymbolAtLocation(
      symbol,
  ...
  Parser.prototype.getReturnDescription = function (symbol) {
    ...
    var returnTag = tags.find(function (tag) {
  ...
  Parser.prototype.getPropsInfo = function (propsObj, defaultProps) {
    ...
    var propsType = this.checker.getTypeOfSymbolAtLocation(
      propsObj,
    ...
    if (propsType.isUnionOrIntersection()) {
      propertiesOfProps = (propertiesOfProps = this.checker.getAllPossiblePropertiesOfTypes(
        propsType.types
      )).concat(baseProps);
      if (!propertiesOfProps.length) {
        var subTypes = this.checker.getAllPossiblePropertiesOfTypes(
          propsType.types.reduce(
            // @ts-ignore
            function (all, t) {
              return all.concat(t.types || []);
            },
            []
          )
        );
    ...
    propertiesOfProps.forEach(function (prop) {
      ...
      var propType = _this.checker.getTypeOfSymbolAtLocation(
        prop,
        propsObj.valueDeclaration
      );
      ...
      var baseProp = baseProps.find(function (p) {
  ...
  Parser.prototype.getFullJsDocComment = function (symbol) {
    ...
    var mainComment = ts.displayPartsToString(
  ...
  Parser.prototype.getFunctionStatement = function (statement) {
    ...
    if (ts.isVariableStatement(statement)) {
      ...
      if (initializer && ts.isCallExpression(initializer)) {
        ...
        if (!symbol_1 || symbol_1.getName() !== "forwardRef") return;
  ...
  Parser.prototype.extractDefaultPropsFromComponent = function (
    symbol,
    source
  ) {
    var _this = this;
    ...
    return possibleStatements.reduce(function (res, statement) {
      if (statementIsClassDeclaration(statement) && statement.members.length) {
        ...
        while (ts.isIdentifier(initializer)) {
          var defaultPropsReference = _this.checker.getSymbolAtLocation(
            initializer
          );
...
function computeComponentName(exp, source) {
  ...
  var statelessDisplayName = getTextValueOfFunctionProperty(
    exp,
    source,
    "displayName"
  );
...
function parseWithProgramProvider(
  ...
  return filePaths
    ...
    .reduce(function (docs, sourceFile) {
      ...
      []);
      var filteredComponentDocs = componentDocsNoDuplicates.filter(function (
        comp,
        index,
        comps
      ) {
